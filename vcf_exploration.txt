...
Now that we have some experience in R, we will check out a vcf file with
polymorphisms from
...
Now we load the VariantAnnotation package as well as the data. The
objective of this exercise is to compare the quality of called SNPs that
are located in dbSNP, versus those that are novel.
...
(information about the content of the file) using scanVcfHeader .
Discover the ‘info’ fields VT (variant type), and RSQ (genotype
imputation quality).
...
SNPs were called with MaCH/thunder (part of GotCloud) , for more info,
see :doc: http://genome.sph.umich.edu/wiki/Thunder and
http://genome.sph.umich.edu/wiki/MaCH\_FAQ. Notice that the seqnames
(chromosome levels) are set to '22', we want to rename those
...
If we compare vcf and vcf\_filt, we see that of the 10376 SNPs in our
initial vcf file, 794 are in the database.
...
(This won't work in R on EC2, simply because we can't run X11 through an
ssh connection)
