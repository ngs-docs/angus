...
## New considerations when performing GWAS on sequecing data
...
## Advantages when performing GWAS on sequecing data
...
isogenic nematode, *Caenorhabditis elegans*, strains from the Million Mutation Project (Thompson *et. al., Genome Research*, 2013) called 
...
for file in *.vcf; do bgzip -c $file > $file.gz; tabix -p vcf $file.gz; done
...
grep -v -E '##' MMP_vcf-merge.vcf | cut -f 10-15 | head -5
```
...
```{r engine='bash'}
# replace  "." with "0/0""
awk '{gsub("\t[.]","\t0/0",$0); print;}' MMP_vcf-merge.vcf > MMP.vcf
grep -v -E '##' MMP.vcf | cut -f 10-15 | head -5
```
...
introns from our analysis. To do this we need to create a snp set ID (`.SSID`) file which indicates which variants 
belong to which set (*e.g.,* gene). `SSID` files contain 2 columns, the first being the gene name and the second being
the variant name. Note - you can define these SNP sets however you want for your own analysis. For example, you could
define them by biochemical pathways as opposed to genes to potentially increase power.
...
```{r engine='bash'}
# Bash Shell
grep -E -v '#' data/MMP.vcf | grep -E 'SN=[A-Z0-9.]{4,}|CODING=' | grep -E -v 'SN=I;' | cut -f 1-8 > data/gene_variants.txt
...
# calculate q-values
qvals <- fdrtool(All_SKAT_Data$results$P.value, statistic = "pvalue", cutoff.method="fndr")
All_SKAT_Data$results$Q.value <- qvals$qval

# sort SKAT results by q-value
head(All_SKAT_Data$results[order(All_SKAT_Data$results$Q.value),])

#save SKAT results
write.table(file = All_SKAT_Data$results, row.names = FALSE, col.names = TRUE, quote = FALSE, append = FALSE)
```
...
MAC_hist <- hist(x <- MACs[,2])
plot(MAC_hist$counts, xlab = "Minor Allele Count", ylab = "Number of Genes")
```
...
# make SSID file with only those variants (i.e. reduce the SSID list to only those with >= 7 variants)
genes_to_include <- data.frame(SSID_count[SSID_count$freq >= 7, 1])
...
# open .SSD and .info files we just created
SSD.info <- Open_SSD('data/MMP-reduced.SSD', 'MMP-reduced.info')

# null model doesn't take in info from .SSID, so we don't need to run that again
# perform SKAT on all sets of variants
All_SKAT_Data_reduced  <- SKAT.SSD.All(SSD.info, Null_Model_reduced) 

# sort SKAT results by p-value
head(All_SKAT_Data_reduced$results[order(All_SKAT_Data_reduced$results$P.value),])

# calculate q-values
qvals <- fdrtool(All_SKAT_Data_reduced$results$P.value, statistic = "pvalue", cutoff.method="fndr")
All_SKAT_Data_reduced$results$Q.value <- qvals$qval

# sort SKAT results by q-value
head(All_SKAT_Data_reduced$results[order(All_SKAT_Data_reduced$results$Q.value),])

#save results
write.table(file = All_SKAT_Data_reduced$results, row.names = FALSE, col.names = TRUE, quote = FALSE, append = FALSE)
```
...
In genetically manipuable model organisms you could instead experimentally confirm that mutations in these 
